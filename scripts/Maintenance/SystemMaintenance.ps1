...
function System-Maintenance {
    ...
    param (
        [System.Management.Automation.SwitchParameter]$Cleanup,
        [System.Management.Automation.SwitchParameter]$Monitor,
        [System.Management.Automation.SwitchParameter]$Backup,
        [System.String]$LogPath = "$env:USERPROFILE\Desktop\SystemMaintenance.log",
        [System.String]$ConfigPath = "$PSScriptRoot\maintenance.config.json",
    ...
    $isElevated = ([System.Security.Principal.WindowsPrincipal][System.Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([System.Security.Principal.WindowsBuiltInRole]::Administrator) ? $true : $false
    ...
    try {
        Write-StandardLog -Message 'Started maintenance' -Level 'INFO' -Path $LogPath
        $report = ''
